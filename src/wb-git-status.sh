#!/usr/bin/env bash
SHOW_WIDGET=$(tmux show-option -gv @catppuccin-tmux_show_wbg)
...
WB_STATUS="#[fg=${THEME[subtext1]},bg=${THEME[base]},bold] $RESET$PROVIDER_ICON $RESET$PR_STATUS$REVIEW_STATUS$ISSUE_STATUS$BUG_STATUS"
