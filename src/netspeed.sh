...
# Check if enabled
ENABLED=$(tmux show-option -gv @catppuccin-tmux_show_netspeed 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
INTERFACE=$(tmux show-option -gv @catppuccin-tmux_netspeed_iface 2>/dev/null)
# Show IP address
SHOW_IP=$(tmux show-option -gv @catppuccin-tmux_netspeed_showip 2>/dev/null)
# Time between refresh
TIME_DIFF=$(tmux show-option -gv @catppuccin-tmux_netspeed_refresh 2>/dev/null)
TIME_DIFF=${TIME_DIFF:-1}

# Icons
declare -A NET_ICONS
NET_ICONS[wifi_up]="#[fg=${THEME[text]}]\U000f05a9"  # nf-md-wifi
NET_ICONS[wifi_down]="#[fg=${THEME[red]}]\U000f05aa"       # nf-md-wifi_off
NET_ICONS[wired_up]="#[fg=${THEME[text]}]\U000f0318" # nf-md-lan_connect
...
NET_ICONS[ip]="#[fg=${THEME[text]}]\U000f0a5f"       # nf-md-ip
...
RX_SPEED="#[fg=${THEME[text]}]$(readable_format "$RX_DIFF" "$TIME_DIFF")"
TX_SPEED="#[fg=${THEME[text]}]$(readable_format "$TX_DIFF" "$TIME_DIFF")"
