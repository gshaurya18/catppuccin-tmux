...
SHOW_GIT=$(tmux show-option -gv @catppuccin-tmux_show_git)
if [ "$SHOW_GIT" == "0" ]; then
  exit 0
fi
...
if [[ $UNTRACKED_COUNT -gt 0 ]]; then
  STATUS_UNTRACKED="${RESET}#[fg=${THEME[overlay2]},bg=${THEME[base]},bold] ${UNTRACKED_COUNT} "
fi
...
case "$SYNC_MODE" in
1)
  REMOTE_STATUS="$RESET#[bg=${THEME[base]},fg=${THEME[red]},bold]▒ 󱓎"
  ;;
2)
  REMOTE_STATUS="$RESET#[bg=${THEME[base]},fg=${THEME[red]},bold]▒ 󰛃"
  ;;
3)
  REMOTE_STATUS="$RESET#[bg=${THEME[base]},fg=${THEME[pink]},bold]▒ 󰛀"
  ;;
*)
